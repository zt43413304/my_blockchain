...
logger = logging.getLogger("my_epay_phone.py")
logger.setLevel(logging.INFO)  # Log等级总开关
# 第二步，创建一个handler，用于写入日志文件
fh = logging.FileHandler('./logs/my_epay_phone.log', mode='w')
...
def epay_login(account_id):
    ...
    headers = {
        ...
        'User-Agent': "ESHOP/8 CFNetwork/978.0.7 Darwin/18.6.0",
        'Accept-Encoding': "br, gzip, deflate",
        'Accept': "*/*",
        'Cache-Control': "no-cache",
        'Postman-Token': "9f9a8e77-b742-405a-9aca-ec41d2afc72b,2784b5f6-c8a8-4268-a31d-761c25d94f54",
        'Host': "epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com",
    }
...
def epay_logout(token):
    ...
    headers = {
        ...
        'Accept': "*/*",
        'Cache-Control': "no-cache",
        'Postman-Token': "077ec063-7c28-45a2-9a47-4ea016fd8ff6,de8e170c-726c-4047-b5c2-6a2edc54e06e",
        'Host': "epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com",
    }
...
def epay_send_captcha_mail(token):
    ...
    try:
        # logger.warning("********** epay_send_captcha_mail()")
        ...
        res = response.json()["code"]
        if res == 200:
            logger.warning('********** epay_send_captcha_mail() success.')

            return 0
        else:
            logger.warning('********** epay_send_captcha_mail() fail.')
            return -1
    except Exception as e:
        print(e)
        return -1
...
def epay_send_captcha_mobile(token):
    url = "https://epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com/pub/user/mobile/send_captcha"
    ...
    headers = {
        ...
        'Accept': "*/*",
        'Cache-Control': "no-cache",
        'Postman-Token': "274d237e-bcd8-4346-b495-ab0a984ebb2c,13be8b43-d2cd-490d-b44a-afe3490993f4",
        'Host': "epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com"
    }
    ...
    try:
        # logger.warning("********** epay_send_captcha_mobile()")
        ...
        res = response.json()["code"]
        if res == 200:
            logger.warning('********** epay_send_captcha_mobile() success.')

            return 0
        else:
            logger.warning('********** epay_send_captcha_mobile() fail.')
            return -1
    except Exception as e:
        print(e)
        return -1
...
def epay_validate_captcha(token, captcha_mno, captcha_mail):
    url = "https://epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com/pub/user/double/validate_captcha"
    ...
    headers = {
        ...
        'Accept': "*/*",
        'Cache-Control': "no-cache",
        'Postman-Token': "395141b0-b112-444e-a353-93fecf9ea3bd,8525bacc-46a0-49fa-a86a-218f2bd3e74b",
        'Host': "epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com",
    }

    payload = "------WebKitFormBoundary7MA4YWxkTrZu0gW\r\n" \
              "Content-Disposition: form-data; name=\"mobile_captcha\"\r\n\r\n" + captcha_mno + "\r\n" \
                                                                                                "------WebKitFormBoundary7MA4YWxkTrZu0gW\r\n" \
                                                                                                "Content-Disposition: form-data; name=\"mail_captcha\"\r\n\r\n" + captcha_mail + "\r\n" \
                                                                                                                                                                                 "------WebKitFormBoundary7MA4YWxkTrZu0gW--"
...
def epay_do_transfer(token, amount):
    ...
    headers = {
        ...
        'Accept': "*/*",
        'Cache-Control': "no-cache",
        'Postman-Token': "7a8b9266-15d7-4f8d-b45e-b097997d0c65,090b4061-8469-44ee-a743-d86312974ad2",
        'Host': "epkkpd5dai.execute-api.ap-northeast-1.amazonaws.com",
    }
...
def epay_transfer(token, amount):
    ...
    try:
        ...
        if res == 110:
            ...
            # 收取邮件验证码
            captcha_mail = send_email.get_email_epay(mail, "epay1234")
            logger.warning('>>>>>>>>>> 邮箱验证码. captcha = ' + captcha_mail)

            if len(str(captcha_mail)) != 6:
                time.sleep(30)
                captcha_mail = send_email.get_email_epay(mail, "epay1234")
                logger.warning('>>>>>>>>>> 第二次取邮箱验证码. captcha = ' + captcha_mail)

            # # 收取手机验证码
            time.sleep(1)
            logger.warning('\n')
            captcha_mmo = input("********** Enter your Mobile Captcha " + mno + " :")
            logger.warning('>>>>>>>>>> 手机验证码. captcha = ' + captcha_mmo)

            # 校验验证码
            return_code = epay_validate_captcha(token, captcha_mmo, captcha_mail)
...
def loop_epay_phone(filename):
    global mail_subject
    # total_bx_all = 0
    # today_bx_all = 0

    # start
    logger.warning('********** Start from loop_epay_phone() ...')
    ...
    for item in data_dict['data']:
        ...
        logger.warning('\n')
        pause = input(">>>>>>>>>> Enter for continue...... ")
...
loop_epay_phone("my_epay_data_phone.json")
